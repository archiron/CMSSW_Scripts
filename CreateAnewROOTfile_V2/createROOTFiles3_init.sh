...
if [[ "$Choice" == "LLR" ]] 
  then
    ...
    do
      ...
      . createROOTFilesS3.sh $i $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED $N_skip & > ~/RoutFichier.log 2>&1 #&
      . createROOTFilesS3.sh $j $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED $N_skip & > ~/RoutFichier.log 2>&1 #&
