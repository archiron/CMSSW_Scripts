...
if [[ "$Choice" == "LLR" ]] 
  then
    ...
    do
      ...
      . createROOTFilesS2.sh $i $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED $N_skip & > ~/RoutFichier.log 2>&1 #&
      . createROOTFilesS2.sh $j $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED $N_skip & > ~/RoutFichier.log 2>&1 #&
